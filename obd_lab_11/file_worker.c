...
const char universityTable[] = "university";
const char facultyTable[] = "faculty";
...
void formFileName(const char tableName[], char fileName[], int fileType) {
    ...
    strcpy(fileName, "");
    char prefix[] = "/Users/danilandreev/Documents/C++ projects/obd_lab_11/";
...
int getItemSize(const char tableName[]) {
    if(strcmp(tableName, universityTable) == 0) {
        return sizeof(University);
    } else if(strcmp(tableName, facultyTable) == 0) {
        return sizeof(Faculty);
    } else {
        return 0;
    }
}
...
bool deleted(void* item, const char tableName[]) {
    if(strcmp(tableName, universityTable) == 0) {
        return ((University*) item)->deleted;
    } else if(strcmp(tableName, facultyTable) == 0) {
        return ((Faculty*) item)->deleted;
    }
    return true;
}
...
void setDeleted(void* item, const char tableName[], bool deleted) {
    if(strcmp(tableName, universityTable) == 0) {
        ((University*) item)->deleted = deleted;
    } else if(strcmp(tableName, facultyTable) == 0) {
        ((Faculty*) item)->deleted = deleted;
    }
}
